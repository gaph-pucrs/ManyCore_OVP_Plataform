...
PPM_REG_READ_CB(regRd32) {
    ...
    if (BHM_DIAG_HIGH) {
        bhmMessage("I", PREFIX, "regRd32 0x%08x (%u)", data, data);
    }
    return data;
}
...
PPM_REG_READ_CB(regRd8) {
    if (BHM_DIAG_HIGH) {
        bhmMessage("I", PREFIX, "regRd8 0x%02x (%u)", *(Uns8*)user, *(Uns8*)user);
    }
    return *(Uns8*)user;
}
...
PPM_REG_WRITE_CB(regWr32) {
    *(Uns32*)user =  byteSwap(data);
    if (BHM_DIAG_HIGH) {
        bhmMessage("I", PREFIX,"regWr32 0x%08x (%u)", *(Uns32*)user, *(Uns32*)user);
    }
...
{
    bhmMessage("F", PREFIX , "Failed to intercept initSemiHost");
...
{
    bhmMessage("F", PREFIX , "Failed to intercept transferDataNative");
...
static void channelThread(void *user)
{
    Uns32 ch = (Uns32) user;
    for (;;) {
        if (BHM_DIAG_MEDIUM) bhmMessage("I", PREFIX, "ch %u waiting", ch);
        DMAState.ch[ch].busy = False;

        bhmWaitEvent(DMAState.ch[ch].start);
        if (DMAState.inReset) {
            if (BHM_DIAG_MEDIUM) bhmMessage("I", PREFIX, "In reset");
        } else {
            DMAState.ch[ch].busy = True;
            if (BHM_DIAG_MEDIUM) bhmMessage("I", PREFIX, "ch %u running", ch);
...
void userInit(void)
{
    ...
    if(readNativeDataChannel != -1) {
        ...
        if(initSemiHost( (Uns32)ch[readNativeDataChannel], nativeDiag)) {
            if (BHM_DIAG_LOW)
                bhmMessage("I", PREFIX ,"Semihost Initialised (ch %d, diag %d)",
                        readNativeDataChannel, nativeDiag);
        }
    }
}
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    // get 'enableNative' parameter
    if (bhmUns32Attribute("enableNativeChannel", &readNativeDataChannel)) {
        if (BHM_DIAG_LOW)
            bhmMessage("I", PREFIX ,"Use Native data on channel %d", readNativeDataChannel);
    }
