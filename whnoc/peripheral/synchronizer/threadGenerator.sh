...
bhmThreadHandle=${bhmThreadHandle}$goThread
...
do
	...
	echo "int _tick"$i" = 1;" >> syncThreads.h
	echo "bhmWaitEvent(goEvent);" >> syncThreads.h
        echo "ppmPacketnetWrite(handles.tickPort_"$i", &_tick"$i", sizeof(_tick"$i"));" >> syncThreads.h
	echo "bhmDeleteThread(th"$i");" >> syncThreads.h
	echo "}" >> syncThreads.h
done

echo "" >> syncThreads.h
echo "void goThread(void *user){" >> syncThreads.h
echo "	bhmWaitDelay(100);" >> syncThreads.h
echo "	bhmTriggerEvent(goEvent);" >> syncThreads.h
echo "}" >> syncThreads.h
