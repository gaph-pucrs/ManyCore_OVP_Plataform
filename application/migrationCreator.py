...
def insertApps(string, fileTipe):
   ...
   for i in range(starts, starts+len(dynamic_app_names)):                    #insert the new include  files
      ...
      if i < ends:
          ...
          lines.insert(i, str)
      j+=1
   if ends > starts+len(dynamic_app_names):                                  #checks if there was more includes than actual scenario
...
with open(filename, 'r') as stream:
    try:
        data_loaded = yaml.safe_load(stream)  # data loaded now is a dictionary with a list of all the apps
