...
static OP_CONSTRUCT_FN(moduleConstructor) {
    ...
    opPeripheralNew(
        ...
        OP_CONNECTIONS(
            OP_BUS_CONNECTIONS(
                OP_BUS_CONNECT(mainBus_b, "bport1", .slave=1, .addrLo=0x100003f8ULL, .addrHi=0x100003ffULL)
            )
        ),
        OP_PARAMS(
             OP_PARAM_STRING_SET("outfile", "v")
            ,OP_PARAM_UNS32_SET("diagnosticlevel", 3)
        )
    );
