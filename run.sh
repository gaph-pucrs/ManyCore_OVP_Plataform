...
rm -Rfv *.csv
rm -fv ovp_compiler.sh
rm -Rfv module/*.so
rm -fv harness/harness.Linux64.exe

echo "> Creating ovp_compiler.sh script"
...
do
    ...
    echo "     --program cpu"$(($N))"=application/application"$(($N))".\${CROSS}.elf \$* --imperasintercepts --parallel --parallelmax  \\" >> ovp_compiler.sh
